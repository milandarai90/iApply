        ...
        <div id="table">
            <table class="text-center table table-bordered table-hover mt-2">
                <tr>
                    ...
                    @foreach ($viewCountry as $index => $item)
                <tr>
                    <td>{{$index+1}}</td>
                    <td class="text-danger">{{$item->name}}</td>
                    <td><a href="" class="text-success">
                    <img src="{{asset('storage/' . $item->map)}}" class=" p-3 border" alt="{{$item->name}}.img" style="height:150px; width:180px; border-radius:4px;">
                    </a></td>
                    <td>
                        <a href="{{route('superadmin.addGeneralCountryGuidelines')}}?id={{$item->id}}"><span class="text-success me-1">Add</span></a>
                        <a href="{{route('superadmin.viewGeneralCountryGuidelines')}}?id={{$item->id}}"><span>View</span></a>
                    </td>
                    <td >
...
@else
    <div class="container">
        <div class="mt-3 d-flex justify-content-center">
            <h5 class="text-danger">No country available. Please add first.</h5>
        </div>
    </div>
