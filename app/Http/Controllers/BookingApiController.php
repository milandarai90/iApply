...
class BookingApiController extends Controller
{
    ...
    //                     'students' => $data->bookingRequest_to_consultancy->studentInfos,
    //                 ];
    //             });
    ...
    public function AfterBookingPage()
    {
         if (Auth::guard('sanctum')->check()) {
            ...
            ->join('users as ciu', 'ci.id', '=', 'ciu.consultancy_id')
            ->join('consultancy_branches as cb', 'si.branch_id', '=', 'cb.id')
            ->join('users as cbu', 'cb.id', '=', 'cbu.branch_id')
            ->join('classrooms as cr', 'si.classroom_id', '=', 'cr.id')
            ->join('courses as cs', 'si.course_id', '=', 'cs.id')
                        ->select('si.status', 'cr.class_name as classroom_name','cs.course as course_name','cbu.name as branch_name','ciu.name as consultancy_name')->where('user_id',$user->id)->get();
    ...
    public function viewBookingRequest(){
        // $user = User::with('consultancy')->find(Auth::id());
        $bookingData = BookingRequest::where('branch_id',Auth::user()->branch_id)
        ->where('status','book')
        ->with('bookingRequestToUser','bookingRequest_to_course','bookingRequest_to_classroom')
        ->get();
    //     dd(BookingRequest::where('branch_id',Auth::user()->branch_id)
    // ->get());
